...
class _ProfilePageState extends State<ProfilePage> {
  ...
  /// 构建设置分区：帮助 → 通用设置 → 推送通知 → 固件升级 → 关于。
  /// 「通用设置」与「推送通知」互换位置，「账号与安全」已删除。
  Widget _buildSettingsSection(AppLocalizations l10n, bool isDark) {
    return _card(
      child: Column(
        children: [
          // 帮助与反馈
          _listItem(Icons.help_outline, l10n.helpFeedback,
              () => Navigator.push(context,
                  MaterialPageRoute(builder: (_) => const HelpFeedbackPage())), isDark: isDark),
          _buildRowDivider(isDark),
          // 通用设置（原在第5位，与推送通知互换后提到第2位）
          _listItem(
              ...
              isDark: isDark),
          _buildRowDivider(isDark),
          // 推送通知（原在第2位，与通用设置互换后移到第3位）
          _listItem(Icons.notifications_outlined, l10n.pushNotifications,
              () => Navigator.push(context,
                  MaterialPageRoute(builder: (_) => const NotificationPage())), isDark: isDark),
          _buildRowDivider(isDark),
          // 固件升级
          _listItem(Icons.system_update_outlined, l10n.firmwareUpdate,
              () => Navigator.push(context,
                  MaterialPageRoute(builder: (_) => const FirmwareUpdatePage())), isDark: isDark),
          _buildRowDivider(isDark),
          // 关于
          _listItem(Icons.info_outline, l10n.aboutThl,
              () => Navigator.push(context,
